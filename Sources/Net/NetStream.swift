...
#if os(iOS) || os(macOS)
    open func attachCamera(_ camera: AVCaptureDevice?, onError: ((_ error: NSError) -> Void)? = nil, userInitiated: Bool = false) {
        lockQueue.async {
            do {
                try self.mixer.videoIO.attachCamera(camera, userInitiated: userInitiated)
